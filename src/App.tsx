...
import DirectionPad, {canVibrate} from './DirectionPad';
...
import './mobile.css';
import { updatePassiveNotice, expirePassiveNotice, type PassiveNotice } from './passiveNotice';

const touchLayoutQuery = '(max-width: 700px), (pointer: coarse) and (max-width: 1200px)';
...
function Icon({ kind }: { kind: 'audio' | 'muted' | 'pause' | 'play' | 'inspect' | 'close' | 'arrow' | 'key' | 'check' | 'restart' | 'map' | 'missions' }) {
  const paths: Record<string, ReactNode> = {
    ...
    missions: <><path d="M8 6h13M8 12h13M8 18h13"/><path d="M3 6h.1M3 12h.1M3 18h.1" strokeWidth="3"/></>,
...
export default function App() {
  ...
  const [touchLayout,setTouchLayout]=useState(()=>matchMedia(touchLayoutQuery).matches);
  const [overview,setOverview]=useState(()=>!matchMedia(touchLayoutQuery).matches);
  useEffect(()=>{
    const media=matchMedia(touchLayoutQuery);
    const update=()=>{setTouchLayout(media.matches);setOverview(!media.matches);};
  ...
  const [hapticsEnabled,setHapticsEnabled]=useState(()=>savedBoolean('ghost-protocol-haptics',true));
  ...
  const [passiveNotice, setPassiveNotice] = useState<PassiveNotice | null>(null);
  ...
  const acceptState = useCallback((next: GameState, audible = true) => {
    ...
    setPassiveNotice(current => updatePassiveNotice(current, next, noticeNow));
  ...
  useEffect(() => {
    if (passiveNotice?.expiresAt == null) return;
    const timer = setTimeout(() => {
      setPassiveNotice(current => expirePassiveNotice(current, performance.now()));
    }, Math.max(0, passiveNotice.expiresAt - performance.now()) + 1);
    return () => clearTimeout(timer);
  }, [passiveNotice]);

  useEffect(()=>{saveBoolean('ghost-protocol-haptics',hapticsEnabled);},[hapticsEnabled]);
  ...
  const openMission=useCallback(async(id:number,showIntro=false)=>{
    if(await command({type:'select-level',levelId:id})){
      ...
      setIntroRequested(showIntro||(id===1&&!progress.introSeen));setScreen('briefing');setOverview(!touchLayout&&id<=2);
    }
  },[command,progress.introSeen,touchLayout]);
  ...
  return <main className={`game ${touchLayout?'touch-layout':''} ${reducedMotion?'reduced-motion':''} ${isTitle?'on-title':''} ${screen!=='game'?'campaign-overlay-open':''} ${overview?'':'following-drone'}`}>
    <div className="scene-shell" aria-label="An isometric training network with maze corridors, access gates and patrolling sentries.">{state&&<Scene state={state} touchLayout={touchLayout} overview={overview} reducedMotion={reducedMotion} onReady={setFps}/>}</div>
    ...
    <header className="topbar"><div className="brand" aria-label="Ghost Protocol"><span className="brand-mark" aria-hidden="true"><i/><i/></span><span>GHOST<br/>PROTOCOL</span></div><div className="mission-heading"><span className="mission-index">{String(level.id).padStart(2,'0')}</span><span className="mission-slash">/</span><span>{level.title.toUpperCase()}</span></div><nav className="top-actions" aria-label="Game settings">
      <button className="quiet-button missions-button" aria-label="Missions" onClick={()=>void openMissions()} disabled={!state||busy||screen==='briefing'}><i className="touch-only"><Icon kind="missions"/></i><span>Missions</span></button>
      ...
      <button className="quiet-button" onClick={()=>setOverview(value=>!value)} disabled={screen!=='game'} aria-label={overview?'Follow drone':'Show whole maze'}><i className="touch-only"><Icon kind="map"/></i><span>{overview?'Follow drone':'Whole maze'} <kbd>M</kbd></span></button>
      ...
      <button className="quiet-button" disabled={!state||isTitle||screen!=='game'||state.status==='won'||state.status==='lost'||inspector||restartConfirm||busy} aria-label={state?.status==='paused'?'Resume':'Pause'} onClick={()=>onAction({type:state?.status==='paused'?'resume':'pause'})}><Icon kind={state?.status==='paused'?'play':'pause'}/><span>{state?.status==='paused'?'Resume':'Pause'}</span></button>
      <button className="quiet-button" disabled={!state||screen!=='game'||restartConfirm||busy} onClick={()=>void toggleInspector()} aria-label="Inspect" aria-expanded={inspector}><Icon kind="inspect"/><span>Inspect</span></button>
    </nav></header>
    {state&&!isTitle&&screen==='game'&&objective&&<aside className="objective-card" aria-label="Current objective"><div className="eyebrow"><span className="mint-line"/>YOUR OBJECTIVE<span className="objective-number">{objective.step}/05</span></div><h1>{objective.title}</h1><p>{touchLayout?objective.text.replace(/press E|Press E/g,'tap the action button'):objective.text}</p><div className="objective-target"><span className="target-diamond"/>{objective.tag}</div>{state.carrying&&<div className="package-carried"><span className="package-icon"/>DATA PACKAGE SECURED</div>}<div className="field-note" aria-live="polite" aria-atomic="true">{learningFeedback&&<><span className="field-note-label">THE SECURITY CONNECTION</span><strong>{learningFeedback.term}</strong><p>{learningFeedback.text}</p></>}</div><DeviceGuide dual={level.grants.length>1}/></aside>}
    {state&&!isTitle&&screen==='game'&&<aside className="status-stack" aria-label="Heist status"><div className="grant-stack">{level.grants.map(definition=>{const grant=state.grants.find(g=>g.id===definition.id),remaining=grant?Math.max(0,Math.ceil((grant.expiresAt-state.elapsedMs)/1000)):null;return <div key={definition.id} className={`access-status ${grant?.revoked?'revoked':remaining!==null&&remaining<12?'urgent':''}`}><span className="status-icon"><Icon kind={grant?.revoked?'check':'key'}/></span><div><span className="eyebrow">{definition.id==='transit'?'T · TRANSIT KEY':'V · VAULT KEY'}</span><strong>{!grant?'Find key':grant.revoked?'Revoked':remaining===0?'Expired':<>{remaining}<small> seconds left</small></>}</strong><span className="key-purpose">{grant?.revoked?'Gates reject this key':remaining===0?'Timer ended. Renew this key.':definition.id==='transit'?'Opens the final exit gate':'Opens the archive gates'}</span>{state.carrying&&definition.id===level.compromisedGrantId&&!grant?.revoked&&remaining!==0&&<span className="key-copy-note">The pursuers hold a copy.</span>}</div></div>;})}{level.grants.length>1&&<p className="automatic-key-note">Gates use the matching key automatically.</p>}</div><div className={`sentry-status ${chased?'tracking':''}`}><span className="sentry-dot"/><span>{touchLayout?(chased?`${chased} CHASING`:blocked===state.sentries.length?'LOCKED OUT':`${state.sentries.length} PATROL`):chased?`${chased} IN PURSUIT`:blocked===state.sentries.length?'PURSUERS LOCKED OUT':`${state.sentries.length} ON PATROL`}</span><small>{chased?'They keep moving when you stop.':blocked===state.sentries.length?(compromised?.revoked?'Vault gate requests were denied.':'Expired access. Revoke before extraction.'):'Watch the corners. Avoid dead ends.'}</small></div></aside>}
    ...
    {state&&!isTitle&&!hasModal&&<div className="live-message" aria-live="polite" aria-atomic="true">
      {state.interactionLabel?<button className={`interaction-prompt actionable ${state.contextObjectId==='revocation-console'?'lockdown-prompt':''}`} onClick={()=>void interact()}><kbd>E</kbd><span>{state.interactionLabel}</span></button>:state.context?<div className="interaction-hint">{state.context.replace(/ · E$/,'')}</div>:!interactionFeedback&&deniedTurn===null&&passiveNotice?.expiresAt!=null&&<div className={`message-text ${chased?'alert-text':''}`}>{touchLayout?passiveNotice.message.replace(/press E/gi,'tap the action button'):passiveNotice.message}</div>}
      {interactionFeedback?<div className={`interaction-result ${['denied','interaction-missed'].includes(interactionFeedback.event)?'missed':'success'}`} data-interaction-result={interactionFeedback.event}><span aria-hidden="true">{['denied','interaction-missed'].includes(interactionFeedback.event)?'i':'✓'}</span><span>{touchLayout?interactionFeedback.message.replace(/press E/gi,'tap the action button'):interactionFeedback.message}</span></div>:deniedTurn!==null&&<div className="denial-toast"><span className="denial-symbol">×</span><div><strong>ACCESS DENIED</strong><span>Revocation removed the pursuers’ gate access.</span></div></div>}
    </div>}
    ...
    <footer className="bottom-bar"><div className="touch-steering-note touch-only">Tap or slide to steer · Brake to stop</div><div className="controls" aria-label="Heist controls"><div className="movement-control"><DirectionPad disabled={!active} touchLayout={touchLayout} direction={state?.direction??null} haptics={hapticsEnabled} onSteer={direction=>onAction({type:'move',direction})}/><span>Steer<small>WASD or arrows · keeps moving</small></span></div><span className="control-divider"/><button className="control-action interact-control" disabled={!active||!state?.interactionLabel} onClick={()=>{void audio.unlock();void interact();}}><kbd>E</kbd><span>{state?.interactionLabel|| (touchLayout?'No action nearby':'No interaction nearby')}</span></button><button className="control-action brake-control" aria-label="Brake" disabled={!active} onClick={()=>onAction({type:'wait'})}><kbd className="wide-key">SPACE</kbd><span>Brake</span></button></div><div className="footer-note"><span className="signal-dot"/>{isTitle?'A FICTIONAL NETWORK. REAL ACCESS DECISIONS.':state?.status==='paused'?'SIMULATION PAUSED':'SPACE STOPS YOU. THE SENTRIES KEEP MOVING.'}</div><div className="run-stats"><span className="chip-counter"><i/>{state?.collectedShards.length??0}<small> CHIPS</small></span><div className="turn-counter"><span className="eyebrow">TIME</span><strong>{clockText(state?.elapsedMs??0)}</strong></div></div></footer>
